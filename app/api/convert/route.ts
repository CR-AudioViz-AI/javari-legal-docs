...
export async function POST(request: NextRequest) {
  try {
    ...
    // Save the conversion into legalease_documents AND the platform's unified
    // customer asset folder, same as document generation - a conversion is a
    // real document too, and should show up in the customer's assets.
    const { data: convDoc } = await supabase.from('legalease_documents').insert({
      user_id: user.id,
      document_type: direction,
      title: `${direction === 'legal-to-plain' ? 'Plain English version of' : 'Legal draft from'} ${sourceName}`,
      content: convertedText,
      status: 'draft',
      metadata: { direction, source_name: sourceName, wordCount, cost },
    }).select('id').single()
    ...
    if (convDoc?.id) {
      await supabase.from('user_assets').insert({
        ...
        name: `Converted: ${sourceName}`,
        file_name: `converted-${sourceName}.txt`,
        url: `https://javarilegal.com/api/documents/${convDoc.id}/download?format=txt`,
        file_url: `https://javarilegal.com/api/documents/${convDoc.id}/download?format=txt`,
        mime_type: 'text/plain',
        metadata: { document_id: convDoc.id, direction },
      })
    }
